...
object PropertiesX {
    ...
    if (onlineFile.exists()) {
        ...
        //local
        val externalFile = new File(new File(PropertiesX.getClass.getProtectionDomain.getCodeSource.getLocation.getPath).getParentFile.getAbsolutePath.replace("\\", "/") + "/dbs.properties")
        if (externalFile.exists()) {
            props.load(new BufferedInputStream(new FileInputStream(externalFile)))
        }
        //in jar
        else {
            props.load(new BufferedReader(new InputStreamReader(PropertiesX.getClass.getResourceAsStream("/conf.properties"))))
        }
    }
