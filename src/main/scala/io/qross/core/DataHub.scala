...
import io.qross.jdbc._
import io.qross.util._
import io.qross.util.Output._

import scala.collection.mutable

class DataHub (defaultSourceName: String = JDBConnection.PRIMARY) {
    
    private val gid = DateTime.now.getNumber("yyyyMMddHHmmssSSS")
    
    private val SOURCES = mutable.HashMap[String, JDBC](
        "CACHE" -> SQLite.connectMemoryDatabase,
        "DEFAULT" -> new JDBC(defaultSourceName)
    )
    ...
    def open(connectionName: String): DataHub = {
        if (!SOURCES.contains(connectionName)) {
            SOURCES += connectionName -> new JDBC(connectionName)
        }
        CURRENT = SOURCES(connectionName)
        this
    }
    
    def openMySQL(connectionName: String): DataHub = {
        if (!SOURCES.contains(connectionName)) {
            SOURCES += connectionName -> new MySQL(connectionName)
        }
        CURRENT = SOURCES(connectionName)
        this
    }
    
    def openHive(connectionName: String): DataHub = {
        if (!SOURCES.contains(connectionName)) {
            SOURCES += connectionName -> new Hive(connectionName)
        }
        CURRENT = SOURCES(connectionName)
        this
    }
    
    def openSQLite(fileNameOrFullPath: String): DataHub = {
        if (!SOURCES.contains(fileNameOrFullPath)) {
            SOURCES += fileNameOrFullPath -> new SQLite(fileNameOrFullPath)
        }
        CURRENT = SOURCES(fileNameOrFullPath)
        this
    }
    
    def openLocalDB(): DataHub = {
        if (!SOURCES.contains("LOCALDB")) {
            SOURCES += "LOCALDB" -> new SQLite(gid + "/local.sqlite")
        }
        CURRENT = SOURCES("LOCALDB")
        this
    }
    ...
    def saveAsMySQL(connectionName: String): DataHub = {
        if (!SOURCES.contains(connectionName)) {
            SOURCES += connectionName -> new MySQL(connectionName)
        }
        TARGET = SOURCES(connectionName)
    ...
    def saveAsSQLite(fileNameOrFullPath: String): DataHub = {
        if (!SOURCES.contains(fileNameOrFullPath)) {
            SOURCES += fileNameOrFullPath -> new SQLite(fileNameOrFullPath)
        }
        TARGET = SOURCES(fileNameOrFullPath)
        this
    }
    
    def saveAsLocalDB(): DataHub = {
        if (!SOURCES.contains("LOCALDB")) {
            SOURCES += "LOCALDB" -> new SQLite(gid + "/local.sqlite")
        }
        TARGET = SOURCES("LOCALDB")
        this
    }
    ...
    def saveAsExcel(fileNameOrFullPath: String, sheetName: String = "sheet1"): Excel = {
        Excel(fileNameOrFullPath).writeTable(TABLE, sheetName).save()
    }
    ...
    def takeOut(tableName: String): DataTable = {
        if (BUFFER.contains(tableName)) {
            DataTable.from(BUFFER(tableName))
        }
        else {
            DataTable()
        }
